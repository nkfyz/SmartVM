...
func opGetInput(pc *uint64, interpreter *EVMInterpreter, callContext *callCtx) ([]byte, error) {
	fmt.Printf("Get Input... \n")
	t1, t2 := callContext.stack.pop(), callContext.stack.pop()
	channel_number := int(t1.Uint64())
	input_size := int(t2.Uint64())
	loc:= int64(0)
	for i:=0;;i++{
		mem_ret := callContext.memory.GetPtr(loc*32, 32)
		var t uint256.Int
		val := int(t.SetBytes(mem_ret).Uint64())
		if val == 123456{
			loc = loc + 1
			break
		}
	...
	for i:=0;i<channel_number;i++{
		for j:=0;j<input_size;j++{
			for k:=0;k<input_size;k++{
				...
				var t uint256.Int
				val := int(t.SetBytes(mem_ret).Uint64())
				callContext.register.Set(reg_loc, val)
				loc = loc + 1
				reg_loc = reg_loc + 1
			}
		}
	}
	fmt.Printf("Get input done. \n")
	return nil, nil
}

func opConvolution(pc *uint64, interpreter *EVMInterpreter, callContext *callCtx) ([]byte, error) {
    fmt.Printf("Convolution Layer\n")
        s:=time.Now().UnixNano()
    t1, t2, t3, t4, t5, t6, t7 := callContext.stack.pop(), callContext.stack.pop(), callContext.stack.pop(), callContext.stack.pop(), callContext.stack.pop(), callContext.stack.pop(), callContext.stack.pop()
    input_size := int(t1.Uint64())  //last push
    kernel_size := int(t2.Uint64())
    stride := int(t3.Uint64())
    padding := int(t4.Uint64())
        input_channel_number := int(t5.Uint64())
    output_channel_number := int(t6.Uint64())
    kernel_start := t7
    
    fmt.Printf("\t 1. Input feature map size: %d \n", input_size)
    fmt.Printf("\t 2. Kernel size: %d \n", kernel_size)
    fmt.Printf("\t 3. Stride: %d \n", stride)
        fmt.Printf("\t 3. Padding: %d \n", stride)
    fmt.Printf("\t 4. Input channel number: %d \n", input_channel_number)
    fmt.Printf("\t 5. Output channel number: %d \n", output_channel_number)
    
    kernel_data_size := int64(kernel_size*kernel_size*input_channel_number*output_channel_number)
    kernel, _ := GetStorage(interpreter, callContext, kernel_start, kernel_data_size, output_channel_number, input_channel_number, kernel_size, kernel_size, 4)
    //input_fm_size := int64(input_size*input_size*input_channel_number)
    input_fm, _ := GetRegister(callContext, input_channel_number, input_size, input_size, 3)
    
        output_size := int((input_size - kernel_size + 2 * padding) / stride + 1)
    output_fm := init3dArray(output_channel_number, output_size, output_size)
    
    for i:=0; i<output_size; i++{
        for j:=0; j<output_size; j++{
            for k:=0; k<output_channel_number; k++{
                for m:=0; m<input_channel_number; m++{
                    for p:=0; p<kernel_size; p++{
                        for q:=0; q<kernel_size; q++{
                            output_fm[k][i][j] = output_fm[k][i][j] + kernel[k][m][p][q] * input_fm[m][stride * i + p][stride * j + q];
                        }
                    }
                }
            }
    ...
    SetRegister(callContext, output_fm, nil, output_channel_number, output_size, output_size, 3)
    fmt.Printf("\t Convolution needs: %v ms\n",(time.Now().UnixNano()-s)/1000000)
    return nil, nil
}

func opMaxPooling(pc *uint64, interpreter *EVMInterpreter, callContext *callCtx) ([]byte, error){
    fmt.Printf("Max Pooling Layer\n")
        s:=time.Now().UnixNano()
    t1, t2, t3 := callContext.stack.pop(), callContext.stack.pop(), callContext.stack.pop()
    input_size := int(t1.Uint64()) // last push
    stride := int(t2.Uint64())
    input_channel_number := int(t3.Uint64())
    ...
    for k:=0;k<output_channel_number;k++{
        for i:=0;i<input_size;i++{
            n=0
            for j:=0;j<input_size;j++{
                output_fm[k][m][n] =  max(input_fm[k][i][j], input_fm[k][i+1][j], input_fm[k][i][j+1], input_fm[k][i+1][j+1]);
                n = n + 1
                j = j + 1
            }
            m = m + 1
    ...
    SetRegister(callContext, output_fm, nil, output_channel_number, output_size, output_size, 3)
        fmt.Printf("\t maxPooling needs: %v ms\n", (time.Now().UnixNano()-s)/1000000)
    return nil, nil
}

func opFullConn(pc *uint64, interpreter *EVMInterpreter, callContext *callCtx) ([]byte, error){
        fmt.Printf("Full Connection Layer\n")
        s:=time.Now().UnixNano()
        t1, t2, t3, t4, t5, t6 := callContext.stack.pop(), callContext.stack.pop(), callContext.stack.pop(), callContext.stack.pop(), callContext.stack.pop(), callContext.stack.pop()
        input_size := int(t1.Uint64())
        input_channel_number := int(t2.Uint64())
        output_channel_number := int(t3.Uint64())
        weights_start := t4
        bias_start := t5
        activeFunc := int(t6.Uint64())

        output_size := 1
	
        bias_data_size := int64(output_channel_number)
        _, bias := GetStorage(interpreter, callContext, bias_start, bias_data_size, output_channel_number, output_size, 0, 0, 2)
        weights_data_size := int64(input_channel_number*output_channel_number)
    _, weights := GetStorage(interpreter, callContext, weights_start, weights_data_size, output_channel_number, input_channel_number, 0, 0, 2)
        _, input_fm := GetRegister(callContext, input_channel_number,input_size,  input_size, 2)
        output_fm := init2dArray(output_channel_number, 1)

        for m := 0; m<output_channel_number; m++{
		for n:=0; n<input_channel_number; n++{
                	output_fm[m][0] = output_fm[m][0] + input_fm[n][0] * weights[m][n]			
            	} 
		if (activeFunc == 1){
            		output_fm[m][0] = ReLu(output_fm[m][0] + bias[m][0])			
		}
	}
        SetRegister(callContext, nil, output_fm, output_channel_number, output_size, output_size, 2)
        fmt.Printf("\t Full Connection needs: %v ms\n", (time.Now().UnixNano()-s)/1000000)
 	fmt.Println(output_fm)
        return nil, nil
}
